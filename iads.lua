...
do
    ...
    local function log(tmpl, ...)
        ...
        if __DEV_ENV == true or internalConfig.ENABLE_DEBUG_LOGGING then
    ...
    local function launchInterceptors(target, threatLevel)
        ...
        -- If the group was airborne, we did not launch anything
        if interceptGroup.airborne == true then
            return false
        else
            return true
        end
    ...
    function iads.receiveJamming(source, power, radial)
